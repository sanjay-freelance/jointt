...
function copyDir(source,destPath,callback){
    if(!source){
        throw new Error("Source directory missing");
    }
    if(!destPath){
        throw new Error("Destination directory path missing");
    }

    fs.copy(source,destPath,function(error){
        if(error){
            return console.error(error)
        }else{
            console.log( "COPY: " + source + " copied to " + destPath)
            if(callback){
                callback();
            }
        }
